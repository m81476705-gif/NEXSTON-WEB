...
const STAFF_LABELS = { owner: "Owner", developer: "Developer", administrator: "Administrator" };
const STAFF_MARKS = { owner: "👑 01", developer: "💻 02", administrator: "🛡️ 03" };

async function loadStaff(){
  try{
    const r = await fetch("/api/staff");
    const x = await r.json();
    document.querySelectorAll("#staffGrid .ticket").forEach(card => {
      const role = card.dataset.role;
      const s = x.staff && x.staff[role];
      const name = s && s.name ? s.name : "Vacant";
      const photo = s && s.photo ? `<img class="staff-photo" src="${s.photo}" alt="${escapeHtml(name)}">` : "";
      card.innerHTML = `
        ${photo}
        <span class="ticket-mark">${STAFF_MARKS[role]}</span>
        <h3>${escapeHtml(STAFF_LABELS[role])}</h3>
        <p>${escapeHtml(name)}</p>`;
    });
  }catch(e){}
}
...
async function loadGallery(){
  try{
    ...
    if (!x.gallery || !x.gallery.length) return;
    grid.innerHTML = x.gallery.map(g => `
      <div class="frame" style="padding:0;overflow:hidden">
        <img src="${g.photo}" alt="${escapeHtml(g.caption)}" style="width:100%;height:100%;object-fit:cover;display:block">
      </div>`).join("");
  }catch(e){}
}
loadGallery();
