...
document.getElementById("adminLoginForm").onsubmit=async e=>{
  ...
  if(x.ok){ e.target.reset(); showDash(); }
  else document.getElementById("adminLoginMsg").textContent=x.data.error||"Login failed.";
};
...
async function loadApplications(){
  ...
  for(const a of x.applications){
    ...
    tr.innerHTML=`
      ...
      <td>${escapeHtml(a.username)}<br><small class="muted">${escapeHtml(a.email)}</small></td>
      ...
      <td class="actions-cell">
        <button class="btn small approve-btn" data-id="${a.id}">APPROVE</button>
        <button class="btn small ghost reject-btn" data-id="${a.id}">REJECT</button>
      </td>`;
