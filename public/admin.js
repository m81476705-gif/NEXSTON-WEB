...
const whitelistMsg = document.getElementById("whitelistMsg");
const wlResult = document.getElementById("wlResult");
const wlReasonField = document.getElementById("wlReasonField");

function syncReasonField() {
  wlReasonField.style.display = wlResult.value === "fail" ? "flex" : "none";
}
wlResult.addEventListener("change", syncReasonField);
syncReasonField();

whitelistForm.onsubmit = async (e) => {
  e.preventDefault();
  setMsg(whitelistMsg, "Sending...", false);
  const fd = new FormData(e.target);
  const payload = {
    name: fd.get("name"),
  ...
  const data = await r.json().catch(() => ({}));
  if (!r.ok) {
    setMsg(whitelistMsg, data.error || "Failed to send.", true);
    return;
  }
  setMsg(whitelistMsg, "Sent to Discord!", false);
  e.target.reset();
  syncReasonField();
};
