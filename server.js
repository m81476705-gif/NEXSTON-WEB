...
async function postWhitelistResult({ name, result, reason, realName, discord }) {
  ...
  if (realName) embed.fields.push({ name: "Real Name", value: realName, inline: true });
  if (discord) embed.fields.push({ name: "Discord", value: discord, inline: true });
  if (!isPass && reason) {
...
app.post("/api/whitelist/apply", (req, res) => {
  const { name, realName, age, discord, heardFrom, heardLink, priorRP } = req.body || {};
  if (!name || !name.trim()) return res.status(400).json({ error: "In-game character name is required." });
  if (!realName || !realName.trim()) return res.status(400).json({ error: "Real name is required." });
  if (!discord || !discord.trim()) return res.status(400).json({ error: "Discord username is required." });
  if (!heardFrom) return res.status(400).json({ error: "Please select how you heard about the server." });
  if (heardFrom === "youtube" && (!heardLink || !heardLink.trim())) {
    return res.status(400).json({ error: "Please paste the YouTube video link." });
  }
  if (priorRP !== "yes" && priorRP !== "no") {
    return res.status(400).json({ error: "Please answer the previous roleplay experience question." });
  }
